...
import { Grid, Typography, Button, Menu, MenuItem, Paper, Box } from '@material-ui/core';
...
const DateTimePicker = () => {
  ...
  const handleToDateChange = (date) => {
    const minDate = date.subtract(30, 'days');
    if (date < from) {
      setFrom(minDate);
    }
      ...
      <Grid container alignItems='center'>
        ...
        <Grid item xs={6}>
          <Box flexDirection="row" alignItems="center" >
            ...
            >
              <MenuItem onClick={() => handleClose(TIME.BREAKFAST)}>Breakfast</MenuItem>
              <MenuItem onClick={() => handleClose(TIME.LUNCH)}>Lunch</MenuItem>
              <MenuItem onClick={() => handleClose(TIME.DINNER)}>Dinner</MenuItem>
              <MenuItem onClick={() => handleClose(TIME.MANUALLY)}>Set exact time range</MenuItem>
            </Menu>
            {timeError && <Typography color='error' variant='inline'>Invalid</Typography>}
            {!openTime && chosenTime && <Typography variant='inline'>{chosenTime} time</Typography>}
            {openTime && <>
              <KeyboardTimePicker
                ampm={false}
                variant="inline"
